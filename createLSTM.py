...
def createHistoryLSTM(itemName: str, data: pd.DataFrame, n_input: int, n_features: int, epochs: int, batch_size: int, save: bool, savePlot: bool) -> None:
    ...
    lstm.add(tf.keras.layers.LSTM(80, return_sequences=True, activation='tanh',recurrent_dropout=0.2))
    lstm.add(tf.keras.layers.LSTM(80))
    lstm.add(tf.keras.layers.Dense(20, activation='tanh'))
    lstm.add(tf.keras.layers.Dense(1))
    optimizer = tf.keras.optimizers.Adam(learning_rate=0.00005)
    lstm.compile(loss=rmse_loss, optimizer=optimizer, metrics=[tf.keras.metrics.RootMeanSquaredError(), RSquare()])
    lstm.summary()
